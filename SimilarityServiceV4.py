...
class SimilarityServiceV4:
    # Base path setup
    base_path = os.path.dirname(os.path.abspath(__file__))
    dataset_path = f"{base_path}/cars_Dataset_3/train"
    def __init__(self,model_name):
        self.model_name=model_name
        self.base_model=self.create_base_model(self.model_name)
    ...
    img_dirs = [name for name in os.listdir(dataset_path)]
    ...
    def build_faiss_index(self):
        ...
        folder_path= f"{self.base_path}/{self.model_name}"

        os.makedirs(folder_path, exist_ok=True)
        # Save for later use
        faiss.write_index(index, f"{folder_path}/faiss.index")
        pickle.dump(image_paths, open(f"{folder_path}/image_paths.pkl", "wb"))
    ...
    def load_faiss_index(self):
        ...
        image_paths = pickle.load(open(f"{folder_path}/image_paths.pkl", "rb"))
        return index, image_paths
    ...
    def find_k_similar_images(self, target, k=5):
        folder_path= f"{self.base_path}/{self.model_name}"
        index_path = f"{folder_path}/faiss.index"
...
# base_path = os.path.dirname(os.path.abspath(__file__))
# test_dataset_path = f"{base_path}/cars_Dataset_3/test"
# dataset_path = f"{base_path}/cars_Dataset_3/train"
#
# model = SimilarityServiceV4("vgg16")
# # model.evaluate_similarity_model(test_dataset_path)
# from PIL import Image
# a= model.find_k_similar_images("/Users/odekunleolasubomi/PycharmProjects/Image_similarity_search/pp.jpeg")
# print(a)
